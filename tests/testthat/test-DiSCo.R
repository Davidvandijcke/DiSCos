...
test_that("test that variations of other arguments work",  {
  ...
  # CI_placebo TRUE
  expect_no_error(disco <- DiSCo(df=df, id_col.target=1, t0=t0, CI_placebo=TRUE, CI=TRUE, boot=2, seed=1, num.cores=2))
  expect_true(!is.null(disco$results.periods[[1]]$DiSCo$CI))

  # CI_placebo FALSE
  expect_no_error(disco <- DiSCo(df=df, id_col.target=1, t0=t0, CI_placebo=FALSE, CI=TRUE, boot=2, seed=1, num.cores=2))
  expect_true(is.null(disco$results.periods[[1]]$DiSCo$CI))
