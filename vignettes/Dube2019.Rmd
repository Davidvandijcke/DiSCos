...
`{# fldr <- here::here("R/")} # sapply(paste0(fldr,list.files(fldr)), source)`
...
```{r}
df <- copy(dube)
disco <- DiSCo(df, id_col.target, t0, M=10000, G = 1000, num.cores = 5, permutation = TRUE, CI = TRUE, boots = 1000, graph = TRUE, simplex=TRUE, seed=1)
```
...
```{r}
disco <- DiSCo(dube, id_col.target, G = 250, num.cores = 2, permutation = TRUE, CI = TRUE, boots = 1000, graph = TRUE, q_min = 0.7, q_max=0.95, seed=1, simplex=TRUE)
```
